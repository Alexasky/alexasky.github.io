									...
									<ul class="list-block">
										<?php
											if(the_category_ID(false) == 4){
												$query = new WP_Query('cat=4&orderby=title&order=ASC');
												if( $query->have_posts()){
												    while( $query->have_posts() ){ $query->the_post();
												    ?>
												       <li class="list-block_item"><a class="list-block_item_link" href="<?php echo get_permalink(); ?>"><span><?php the_title(); ?></span></a></li>
												    <?php
												    }
										...
										<?php
											if(the_category_ID(false) == 3){
												$query = new WP_Query('cat=3&orderby=title&order=ASC');
												if( $query->have_posts()){
												    while( $query->have_posts() ){ $query->the_post();
												    ?>
												       <li class="list-block_item"><a class="list-block_item_link" href="<?php echo get_permalink(); ?>"><span><?php the_title(); ?></span></a></li>
